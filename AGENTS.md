...
Scope: repository. Snapshot: 2026-09-18, v0.17.0, source baseline `76786e8`.
...
Add-ons: `AddonManager` in `addons.py` discovers `petey-addon.json` folders under the local data root's `addons/` directory without importing disabled code. Persistent enablement is `DesktopState.settings.addons`; changes require restart. Enabled external entrypoints run `setup(AddonContext)` after core routes/database setup and may add namespaced API routes, optional sidebar panel/assets, services, and a returned `close()` hook. Assets resolve inside the add-on folder; writable state belongs in `addon-data/<id>`. Discord is the only built-in add-on and defaults on. GitHub releases do not bundle third-party/example add-ons; local add-on development folders are ignored. UI/API: `view-addons`, `/api/desktop/addons*`; author contract: `docs/addons.md`. Add-ons are trusted local code with process-level access.
...
Discord: sidebar `view-discord` -> `/api/desktop/discord*` -> injectable `DiscordBridge` worker (`discord_bridge.py`) -> official Discord v10 REST plus `DiscordGateway` presence and interactions (`discord_transport.py`). `room_chat.py` supplies bounded model context and adaptive pacing. `discord_media.py` defines guild slash commands mapped to the desktop `MediaJobManager`; interaction attachments are HTTPS Discord CDN files capped at 25 MiB, jobs use saved provider/model settings, results enter Gallery, and deferred replies are updated before the 15-minute interaction-token limit. Command registration creates/updates PETEY's known commands without deleting unrelated application commands. The OAuth invite requests Administrator (`8`) plus `applications.commands`. Connect starts Gateway Identify with online presence, heartbeat, and reconnect; close stops it. Successful connects persist the selected server/channel; optional auto-connect reuses it at app startup. `DesktopState.settings.discord.room_prompt` is editable/resettable in the Discord UI and applies to the active `RoomConversation` on the next reply; AI enhance returns an unsaved draft from the configured chat provider, and Personality precedes the saved prompt. Owner instructions matching create text channel, delete selected channel, or clear 1–100 recent messages create ten-minute review proposals; `/discord/admin-approve` executes them and public channel text never does. Direct mentions/replies/name matches, a three-minute same-speaker follow-up window, and persistent watched topics bypass warm-up and the Discord message ceiling, with a two-second eligibility cap. Persistent pacing modes govern only spontaneous participation. Native typing starts before generation and refreshes every 8s. REST poll interval 4s. Ignores bots/webhooks and disables outgoing mentions. Bot token stays private with `DISCORD_BOT_TOKEN` fallback. Model/personality snapshot at Connect; no desktop memory/tools. UI: `web/static/discord.js`, `web/templates/desktop_discord.html`; tests `test_discord*.py` and `test_room_chat.py`, API coverage in `test_desktop_app.py`. See `docs/discord.md`.
...
| Add-on discovery/lifecycle/UI contract | `addons.py`: `AddonManager`, `AddonContext`; `desktop_state.py`: `addon_enabled`; API `/addons*`; template `view-addons`; `docs/addons.md`; `tests/test_addons.py` |
...
Settings UI: `web/settings_pages.py` is the page/group catalog. `web/templates/settings/shell.html` renders shared headers/navigation and includes one content template per page. `view-settings` is the overview; Appearance & profile is `view-appearance`. Other stable views: Personality & voice, Microphone, Models & API keys, Tools & integrations, Knowledge files, Memory & privacy. `web/static/settings.js` derives section links/search from `.settings-card[id]`; keep IDs stable. `openSetting` resolves a target's actual page, preserving old section links after moves. `desktop.js` derives routes from `.app-view` and owns feature loading/save handlers. Provider choices/keys stay centralized. Search indexes static headings/labels, never field values. Persona snapshots include speech configuration; microphone behavior stays installation-wide. Theme preference (`midnight`, `ocean`, `forest`, `paper`) is validated/persisted by DesktopState, server-rendered on `<html data-theme>`, and applied by semantic CSS variables. See `docs/interface-design.md`; structure/link tests: `tests/test_settings_ui.py`. `/api/desktop/provider-keys` returns status only; saved deAPI keys feed media and STT with environment fallback.
